...
wisdom_ulid() {
  ...
  # Convert each pair of hex (8 bits) into 8 bits, MSB-first
  local bits="" b pair dec
  for (( i=0; i<${#hex}; i+=2 )); do
    pair="${hex[$((i+1)),$((i+2))]}"
    dec=$((16#${pair}))
    b=$(([##2]dec))
    # zsh [##2] arithmetic prints base-2 without padding; left-pad to 8
    while (( ${#b} < 8 )); do b="0${b}"; done
    bits+="$b"
  done
  local r=""
  for (( i=0; i<80; i+=5 )); do
